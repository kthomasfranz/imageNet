...
## Example Dataset: Airports and Southwest Airlines flights data set
To understand how to use the geoNet function, we’ll use the **airports** and **Southwest** datasets as an example. The ariports data, the data frame of nodes (each corresponding to an airport in this case), contains 2671 observations noting the longitude and the latitude of the airport and a few synthetic sample variables that we will use to show how we can tweak the node size and the node color. The Sothwest data, the data frame of edges (in this case of flights), gives us information on the origin and destination airports for each Southwest flight.  It also provides us with some extra attributes for each edge that we can use to tweak the width and color of each edge.  For more information about the data, please see ?imageNet::Southwest and ?imageNet::airports
...
m <- geoNet(df = flights, geodata = airports, node_color = "red", node_size = 3,
               edge_color = 'cornflowerblue', edge_size = 4, name = "AIRPORT", lat = "LATITUDE",
